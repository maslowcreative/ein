...
class ProviderOldDataCapture extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($plans as $plan){
            ...
            if($lineItems->count()> 0){
                ...
                foreach ($lineItems as $item)
                {
                    ...
                    if($planBudget){
                        ...
                        {
                            $providerBudget->amount = $providerBudget->amount + $amount;
                            $providerBudget->pending = $providerBudget->pending + $pending;
                            $providerBudget->spent = $providerBudget->spent + $spent;
                        }
            ...
            $recLineItems = DB::table('claim_line_items')->where('plan_id',$plan['id'])->whereIn('status',[
